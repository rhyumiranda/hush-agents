...
Every packet must carry a source reference for each requirement it covers, including the exact source digest and location. When Fable marks a requirement `risk: HIGH` with `FABLE-HIGH-RISK`, the packet must include a `mutation_policy` requiring StrykerJS and all applicable authorization, publication, consent, audit, and security checks. Do not weaken that policy to make a task runnable.
