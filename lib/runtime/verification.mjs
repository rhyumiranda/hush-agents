...
/** Run the Stryker version check and mutation command synchronously, then record bound mutation evidence. */
export function runStrykerPolicy(options = {}) {
  const run = prepareStryker(options);
  const version = strykerVersion(spawnSync("npx", STRYKER_VERSION_ARGS, { cwd: run.cwd, env: run.env, timeout: run.timeoutMs }));
  const result = spawnSync("/bin/sh", ["-c", run.command], { cwd: run.cwd, env: run.env, timeout: run.timeoutMs });
  return finishStryker(run, version, { status: result.status, stdout: result.stdout, stderr: result.stderr, timedOut: result.error?.code === "ETIMEDOUT" });
}

/**
 * The same as runStrykerPolicy, but the version check and the mutation run (up to timeoutMs) do not block the event loop.
 * The runner uses this function so that one task's mutation run does not freeze the other parallel tasks.
 */
export async function runStrykerPolicyAsync(options = {}) {
  const run = prepareStryker(options);
  const version = strykerVersion(await spawnCapture("npx", STRYKER_VERSION_ARGS, run));
  const result = await spawnCapture("/bin/sh", ["-c", run.command], run);
  return finishStryker(run, version, result);
}

const STRYKER_VERSION_ARGS = ["--no-install", "stryker", "--version"];

function prepareStryker({ cwd, command, reportPath = "reports/mutation.json", packet, candidate, snapshot, changedPaths, timeoutMs = 120_000, env = {}, root, runId, now, actor = "puck" }) {
  ...
  return { cwd, command: command ?? `npx --no-install stryker run ${mutationArgs.join(" ")}`, reportPath, packet, candidate, snapshot, changedPaths, timeoutMs, env: { ...process.env, ...env, HUSH_NETWORK_POLICY: "disabled" }, root, runId, now, actor };
}

function strykerVersion(result) {
...
function finishStryker({ cwd, command: effectiveCommand, reportPath, packet, candidate, snapshot, changedPaths, root, runId, now, actor }, version, result) {
...
/**
 * Async spawn that returns the same fields runStrykerPolicy reads from spawnSync: status, stdout and stderr as Buffers,
 * and timedOut. On timeout, the child gets SIGTERM, like the spawnSync `timeout` option. After a timeout the function
 * settles on 'exit', so a grandchild process that still holds the pipes cannot delay the result.
 */
function spawnCapture(command, args, { cwd, env, timeoutMs }) {
  return new Promise((resolveResult) => {
    ...
    const settle = (status) => {
      ...
      settled = true;
      clearTimeout(timer);
      resolveResult({ status: timedOut ? null : status, stdout: Buffer.concat(stdout), stderr: Buffer.concat(stderr), timedOut });
    };
    child.stdout.on("data", (chunk) => stdout.push(chunk));
    child.stderr.on("data", (chunk) => stderr.push(chunk));
    child.on("error", () => settle(null));
    // A normal exit waits for 'close', so stdout is complete and its digest matches the sync path. After a timeout,
    // 'exit' is enough. Destroying the streams releases pipes that an orphaned grandchild may still hold open.
    child.on("exit", (status) => { if (timedOut) { child.stdout.destroy(); child.stderr.destroy(); settle(status); } });
