...
export function mutationPolicyForRequirements(requirements = []) {
  ...
  const tagged = highRisk.filter((requirement) => [...(requirement.tags ?? []), ...(requirement.fable_tags ?? [])].some((tag) => String(tag).toUpperCase() === "FABLE" || String(tag).toUpperCase() === "FABLE-HIGH-RISK"));
...
export function runStrykerPolicy({ cwd, command, reportPath = "reports/mutation.json", packet, candidate, snapshot, changedPaths = [], timeoutMs = 120_000, env = {}, root, runId, now, actor = "puck" } = {}) {
  ...
  if (!packet?.packet_id || !candidate?.candidate_id || !candidate?.patch_digest || !snapshot?.snapshot_id || !snapshot?.diff_digest) throw new Error("MUTATION_BINDINGS_REQUIRED");
  if (!Array.isArray(changedPaths) || changedPaths.length === 0) throw new Error("MUTATION_CHANGED_PATHS_REQUIRED");
  const mutationArgs = changedPaths.length ? changedPaths.flatMap((path) => ["--mutate", shellQuote(path)]) : [];
  const effectiveCommand = command ?? `npx --no-install stryker run${mutationArgs.length ? ` ${mutationArgs.join(" ")}` : ""}`;
  const version = readStrykerVersion(cwd, timeoutMs, env);
  const result = spawnSync("/bin/sh", ["-c", effectiveCommand], { cwd, env: { ...process.env, ...env, HUSH_NETWORK_POLICY: "disabled" }, encoding: "utf8", timeout: timeoutMs });
  ...
  let rawReport;
  try { rawReport = JSON.parse(reportBytes.toString("utf8")); } catch (error) { throw Object.assign(new Error(`MUTATION_REPORT_INVALID: ${error.message}`), { code: "MUTATION_REPORT_INVALID" }); }
  ...
  const report = createMutationEvidence({
    ...
    candidate_patch_digest: candidate?.patch_digest ?? null,
    candidate_diff_digest: snapshot?.diff_digest ?? null,
...
export function validateVeraShellCommand(command, { snapshotRoot, cwd = snapshotRoot, packet, approvedCommands } = {}) {
  ...
  if (DENIED_VERA.test(command ?? "")) errors.push("command is not read-only or is outside Vera capability");
  if (SHELL_WRITE.test(command ?? "")) errors.push("shell write operation is forbidden");
...
export function runVeraShell({ command, snapshotRoot, cwd = snapshotRoot, timeoutMs = 30_000, packet, approvedCommands } = {}) {
  ...
  return { command, cwd, exit_status: result.status ?? 1, stdout: result.stdout ?? "", stderr: result.stderr ?? "", stdout_digest: sha256(result.stdout ?? ""), stderr_digest: sha256(result.stderr ?? "") };
}
...
export function validateMutationReport(report, bindings = {}) {
  const errors = [];
  if (bindings.packet?.packet_id !== report?.packet_id) errors.push("packet binding mismatch");
  if (bindings.candidate?.candidate_id !== report?.candidate_id || bindings.candidate?.patch_digest !== report?.candidate_patch_digest) errors.push("candidate binding mismatch");
  if (bindings.snapshot?.snapshot_id !== report?.snapshot_id || bindings.snapshot?.diff_digest !== report?.candidate_diff_digest) errors.push("snapshot binding mismatch");
  if (report?.status !== "PASS" || Number(report?.surviving_mutations ?? 0) !== 0) errors.push("surviving mutation blocks acceptance");
...
export function recordMutationEvidence(root, runId, report, { now, actor = "puck" } = {}) {
  if (!report?.report_id) throw new Error("MUTATION_REPORT_ID_REQUIRED");
...
export function summarizeStrykerReport(report) {
  ...
  const statuses = mutants.map((mutant) => String(mutant.status ?? "UNKNOWN"));
...
function readStrykerVersion(cwd, timeoutMs, env) {
  const result = spawnSync("npx", ["--no-install", "stryker", "--version"], { cwd, env: { ...process.env, ...env, HUSH_NETWORK_POLICY: "disabled" }, encoding: "utf8", timeout: timeoutMs });
  if (result.error?.code === "ETIMEDOUT" || result.status !== 0) throw Object.assign(new Error("MUTATION_TOOL_UNAVAILABLE"), { code: "MUTATION_TOOL_UNAVAILABLE", result });
