...
/**
 * Stable identity of a repository: sha256 of its real git common dir, so every worktree of one repo maps to one id.
 * The common dir is memoized per canonical repo (see gitPathFacts) because pool and queue events call this hundreds of times per run.
 */
export function repositoryIdentity(repo) {
  const canonicalRepo = canonicalRepository(repo);
  const facts = gitPathFacts.get(canonicalRepo);
  if (facts?.identity) return facts.identity;
  const commonDir = git(canonicalRepo, ["rev-parse", "--path-format=absolute", "--git-common-dir"]).stdout.trim();
  const identity = sha256(realpathSync(commonDir));
  gitPathFacts.set(canonicalRepo, { ...gitPathFacts.get(canonicalRepo), identity });
  return identity;
}
...
/** Create a worktree and run the setup profile synchronously. The CLI `worktree-warm` command uses this function. */
export function warmWorktree(options) {
  const warm = beginWarm(options);
  const records = [];
  for (const command of warm.commands) {
    const result = spawnSync("/bin/sh", ["-c", command], { cwd: warm.worktree.absolute_path, env: warm.env, encoding: "utf8" });
    records.push(setupRecord(command, warm.worktree.absolute_path, result));
    if (result.status !== 0) break;
  }
  return finishWarm(warm, records);
}

/**
 * The same as warmWorktree, but it runs the setup commands (for example `npm ci`) without blocking the event loop.
 * The runner uses this function so that parallel tasks do their setup at the same time instead of one after another.
 * All state and pool writes stay synchronous, so concurrent warm-ups never interleave a partial write.
 */
export async function warmWorktreeAsync(options) {
  const warm = beginWarm(options);
  const records = [];
  for (const command of warm.commands) {
    const result = await spawnShell(command, warm.worktree.absolute_path, warm.env);
    records.push(setupRecord(command, warm.worktree.absolute_path, result));
    if (result.status !== 0) break;
  }
  return finishWarm(warm, records);
...
function beginWarm({ repo, baseSha, runId = "WARM", taskId = "BASE", worktreeId, profile, packet, packetPath, timestamp, root = repo }) {
  const canonicalRepo = canonicalRepository(repo);
  ...
  return { canonicalRepo, packetData, digest, worktree, commands, env, root, timestamp };
}

function setupRecord(command, cwd, result) {
  return { command, cwd, exit_status: result.status ?? 1, stdout_digest: sha256(result.stdout ?? ""), stderr_digest: sha256(result.stderr ?? "") };
}

function finishWarm({ canonicalRepo, packetData, digest, worktree, root, timestamp }, commandRecords) {
  const failedCommand = commandRecords.find((record) => record.exit_status !== 0);
  const validation = failedCommand ? { status: "BLOCKED", code: "WARM_SETUP_FAILED" } : validateWorktree({ ...worktree, profile_digest: digest }, packetData);
  if (validation.status !== "VALID") {
    ...
    recordWorktreeEvent(root, worktree.run_id, worktree.worktree_id, "warm-failed", failed, failedCommand ? "setup-command-failed" : validation.code, timestamp);
...
/** Run one shell command without blocking. The result has the same fields that warmWorktree reads from spawnSync. */
...
// ponytail: per-process memo of `git rev-parse` path facts. A measured 4-task run made 537 rev-parse spawns (~3.6s).
// Ceiling: a path re-initialised as a different repo in the same process keeps the old facts; the `.git` stat catches deletion.
// Failures are never cached, so REPOSITORY_REQUIRED still fires on every bad call.
const gitPathFacts = new Map();

/** Resolve any path inside a repo or worktree to the real path of its git toplevel. Memoized per real input path. */
function canonicalRepository(repo) {
  const candidate = realpathSync(resolve(repo));
  const cached = gitPathFacts.get(candidate)?.toplevel;
  ...
  gitPathFacts.set(candidate, { ...gitPathFacts.get(candidate), toplevel });
  return toplevel;
}
