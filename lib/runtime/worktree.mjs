...
export function recoverInterruptedWorktreeLeases(root, runId, { now, owner = "hush" } = {}) {
  ...
  for (const record of Object.values(state.entities.worktree ?? {})) {
    ...
    const next = { ...record, state: "BLOCKED", block_reason: "WORKER_INTERRUPTED", previous_lease_id: record.lease_id, recovered_at: now ?? new Date().toISOString() };
