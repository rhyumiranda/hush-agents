...
export function warmWorktree({ repo, baseSha, runId = "WARM", taskId = "BASE", profile, packet, packetPath, timestamp, root = repo }) {
  const canonicalRepo = canonicalRepository(repo);
  ...
  const worktree = createWorktree({ repo: canonicalRepo, baseSha, runId, taskId, timestamp, root });
...
export function validateWorktree(record, packet = null) {
  ...
  try {
    ...
    if (head !== record.base_sha) return { status: "INVALID", code: "WORKTREE_STALE_BASE", actual: head, expected: record.base_sha };
...
export function releaseWorktree({ root = ".", worktreeId, runId, cleanup = false, now }) {
  ...
  const validation = validateWorktree(record);
  if (validation.status !== "VALID") {
...
export function recoverExpiredWorktreeLeases(root, runId, { now } = {}) {
  ...
  for (const record of Object.values(state.entities.worktree ?? {})) {
    ...
    const next = { ...record, state: "BLOCKED", block_reason: "WORKTREE_LEASE_EXPIRED", previous_lease_id: record.lease_id };
