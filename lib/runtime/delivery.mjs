...
export class GhAxiAdapter {
  ...
  createOrUpdatePr(payload, { prNumber } = {}) {
    ...
    return { provider: "github", command: [GH_AXI_COMMAND, ...args], status: prNumber ? "UPDATED" : "CREATED", response_digest: digest(result.stdout ?? result) , remote: result.stdout ?? result };
  }
...
export function ingestProviderEvent(root, runId, event, { expectedIdentity = {}, now, actor = "hush" } = {}) {
  ...
  if (!identity.valid) {
    const recorded = appendStateEvent(root, runId, { entity_type: "provider_event", entity_id: eventId, action: "rejected", action_key: actionKey, actor, cause: "CI_IDENTITY_MISMATCH", timestamp: now, data: { ...normalized, action_key: actionKey, status: "BLOCKED", errors: identity.errors } });
    return { status: "BLOCKED", event: recorded, action_key: actionKey, errors: identity.errors };
  }
  const recorded = appendStateEvent(root, runId, { entity_type: "provider_event", entity_id: eventId, action: "received", action_key: actionKey, actor, cause: "provider-event", timestamp: now, data: { ...normalized, action_key: actionKey } });
