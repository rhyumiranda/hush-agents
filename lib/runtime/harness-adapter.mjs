...
export function buildHarnessInvocation({ harness, role, payload, outputPath, executable = harness, profileText = "", sandbox, model } = {}) {
  ...
  if (harness === "claude") {
    return { command: executable, args: ["-p", ...modelArgs, "--agent", role, "--output-format", "json", "--no-session-persistence", "--permission-mode", mode === "read-only" ? "plan" : "dontAsk", "--add-dir", workdir, prompt], input: "" };
  }
  if (harness === "gemini") {
