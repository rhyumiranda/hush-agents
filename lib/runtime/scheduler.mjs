...
export const TASK_TRANSITIONS = Object.freeze({
  PLANNED: ["BLOCKED", "READY"], READY: ["RUNNING"], RUNNING: ["READY_FOR_PUCK", "BLOCKED"],
...
export function taskQueue(state, options = {}) {
  ...
  const capacity = capacityPolicy(observation, { baseline: options.baselineWorkers ?? SAFE_BASELINE_WORKERS, hardMax: MAX_WORKERS });
...
export function capacityPolicy(observation, { baseline = SAFE_BASELINE_WORKERS, hardMax = MAX_WORKERS } = {}) {
