...
export const TASK_TRANSITIONS = Object.freeze({
  ...
  READY_FOR_PUCK: ["VERIFIED", "FAILED", "BLOCKED"], VERIFIED: ["READY_FOR_VERA", "ACCEPTED"],
...
export function recoverInterruptedLeases(root, runId, options = {}) {
  ...
  for (const task of Object.values(state.entities.task ?? {})) {
    ...
    const event = appendStateEvent(root, runId, {
      entity_type: "task", entity_id: task.id, action: "worker-interrupted", actor: options.owner ?? "hush", cause: "RUNNER_RESUME",
      timestamp, data: { status: "READY", attempt, retry_reason: "RUNNER_RESUME", previous_lease_id: task.lease_id ?? null, previous_lease_expires_at: task.lease_expires_at ?? null },
    });
    recovered.push({ task_id: task.id, attempt, revision: event.revision, reason: "RUNNER_RESUME" });
  }
  const summary = writeRunSummary(root, runId, { recovered, state: loadSchedulerState(root, runId) });
  return { recovered, summary };
}
