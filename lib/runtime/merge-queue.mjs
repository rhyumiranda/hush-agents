...
export const MERGE_STATES = Object.freeze({
  ...
  INTEGRATING: ["READY_FOR_INTEGRATION", "INTEGRATED", "CONFLICTED", "FAILED", "BLOCKED"],
...
export function processNextMerge({ root = ".", repo, target, itemId, now, integrationChecks = [], owner, finalEvidence } = {}) {
  ...
  try {
    ...
    let queueItem = transitionQueueItem(canonicalRepo, root, item, "INTEGRATING", { target_sha: targetSha, lock_id: lock.lock_id, integration_started_at: timestamp, integration_path: integrationPath(canonicalRepo, item), integration_branch: integrationBranch(item) }, "target-lock-acquired", timestamp);
...
export function recoverIntegrating({ root = ".", repo, target, now, staleMs = MERGE_LOCK_MS } = {}) {
  const canonicalRepo = canonicalRepository(repo);
  const timestamp = now ?? new Date().toISOString();
  const cutoff = Date.parse(timestamp) - staleMs;
  const recovered = [];
  for (const item of queueStatus(canonicalRepo, target).filter((candidate) => candidate.status === "INTEGRATING")) {
    if (Date.parse(item.integration_started_at ?? "") > cutoff) continue;
    const path = item.integration_path ?? integrationPath(canonicalRepo, item);
    cleanupIntegrationPath(canonicalRepo, path, item.integration_branch ?? integrationBranch(item));
    const next = transitionQueueItem(canonicalRepo, root, item, "READY_FOR_INTEGRATION", { recovery_count: number(item.recovery_count, 0) + 1, recovery_reason: "INTEGRATION_INTERRUPTED", recovered_at: timestamp, integration_path: path }, "integration-recovery", timestamp);
    recovered.push(next);
  }
  return { status: recovered.length ? "RECOVERED" : "IDLE", recovered };
}

export function cleanupIntegrationWorktree({ root = ".", repo, itemId, now, force = false } = {}) {
  const canonicalRepo = canonicalRepository(repo);
  const item = queueStatus(canonicalRepo).find((candidate) => candidate.item_id === itemId);
  if (!item) throw new Error("QUEUE_ITEM_NOT_FOUND");
  if (!["READY_FOR_PR", "TARGETED_RECHECK", "CONFLICTED", "FAILED", "BLOCKED", "HUMAN_DECISION"].includes(item.status)) throw new Error("INTEGRATION_CLEANUP_NOT_READY");
  const path = item.integration_path ?? integrationPath(canonicalRepo, item);
  const branch = item.integration_branch ?? integrationBranch(item);
  if (existsSync(path) && !force && workingTreeStatus(path)) throw new Error("INTEGRATION_WORKTREE_DIRTY");
  cleanupIntegrationPath(canonicalRepo, path, branch, { force });
  const cleaned = { ...item, status: item.status, integration_cleaned_at: now ?? new Date().toISOString(), integration_path: path, integration_branch: branch };
  appendQueueEvent(canonicalRepo, "integration-cleaned", cleaned, now);
  appendMergeState(root, item.run_id, item.item_id, "integration-cleaned", cleaned, "integration-cleanup", now);
  return { status: "CLEANED", item: cleaned, path, branch };
}
...
function createIntegrationWorktree(repo, item, targetSha, timestamp) {
  const path = integrationPath(repo, item);
  ...
  const branch = integrationBranch(item);
...
// ponytail: per-process memo of `git rev-parse --show-toplevel`; queue ops call this on every event. Failures are never cached.
const toplevelCache = new Map();
/** Resolve a path to its git toplevel. Memoized per input path; the `.git` stat drops entries for deleted repos. */
function canonicalRepository(repo) {
  const candidate = resolve(repo);
  const cached = toplevelCache.get(candidate);
  if (cached && existsSync(join(cached, ".git"))) return cached;
  ...
  toplevelCache.set(candidate, toplevel);
  return toplevel;
}
...
function integrationPath(repo, item) { return resolve(repo, ".hush", "integration", repositoryIdentity(repo), safeSegment(item.item_id)); }
function integrationBranch(item) { return `hush/integration/${safeSegment(item.item_id)}`; }
function cleanupIntegrationPath(repo, path, branch, { force = true } = {}) {
  if (existsSync(path)) {
    const removed = git(repo, ["worktree", "remove", ...(force ? ["--force"] : []), path]);
    if (removed.status !== 0) throw new Error(`INTEGRATION_CLEANUP_FAILED: ${removed.stderr.trim()}`);
  }
  const branchResult = git(repo, ["branch", "-D", branch]);
  if (branchResult.status !== 0 && !branchResult.stderr.includes("not found")) throw new Error(`INTEGRATION_BRANCH_CLEANUP_FAILED: ${branchResult.stderr.trim()}`);
}
