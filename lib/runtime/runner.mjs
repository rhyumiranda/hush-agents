...
import { dispatchReadyTasks, recordCapacityObservation, routeFailure } from "./scheduler.mjs";
...
import { allocateWorktree, warmWorktree } from "./worktree.mjs";
...
export function runWorkflow(options) {
  let context;
  try {
    return runWorkflowInternal(options, (value) => { context = value; });
  } catch (error) {
    if (context) {
...
function runWorkflowInternal(options, captureContext) {
  ...
  const candidate = latestAcceptedCandidate(state);
  if (!candidate) return finishBlocked(context, RUN_EXIT_CODES.BLOCKED, "CANDIDATE_MISSING", null);
  ...
  appendStateEvent(repository, runId, { entity_type: "delivery", entity_id: `PR-${runId}`, action: "pr-payload-rendered", actor: "hush", cause: "runner-local-integration", timestamp: context.now, data: { status: "READY_FOR_PR", payload, payload_digest: payload.payload_digest, integration_id: integration.integration_record?.integration_id ?? null } });
  appendRunStatus(context, "ACCEPTED", "runner-acceptance", { acceptance_record_id: `ACC-${queue.item.item_id}`, candidate_id: candidate.id, integration_id: integration.integration_record?.integration_id ?? null, pr_payload_digest: payload.payload_digest });
...
function validateRunnerInput(options = {}) {
  ...
  return { ...options, repository: resolvedRepository, prdPath: resolvedPrd, configPath: resolve(options.configPath), target: options.target, maxWorkers: options.maxWorkers ?? 3 };
}
...
function runFable(context) {
  const result = runAdapter(context, "fable", null, { prd_path: relative(context.repository, context.prdPath), prd_digest: context.prdDigest, prd_text: context.prdText, source_manifest: context.source });
...
function runRook(context, requirements) {
  const result = runAdapter(context, "rook", null, { requirements, requirement_ids: requirements.map((item) => item.requirement_id), base_sha: context.baseSha, target: context.target, environment: context.environment });
...
function processTasks(context) {
  ...
  for (let loop = 0; loop < maxLoops; loop += 1) {
    ...
    for (const admission of dispatch.dispatched) processTask(context, admission.task_id);
  }
  throw new RunnerError("runner exceeded its task progress limit", RUN_EXIT_CODES.FAILED);
}

function processTask(context, taskId) {
  const state = replayRunState(context.repository, context.runId);
  ...
  const warm = warmWorktree({ repo: context.repository, baseSha: packet.base_sha, runId: context.runId, taskId, profile: context.setup_profile, packet: stripStateMetadata(packet), root: context.repository, timestamp: context.now });
  ...
  const flint = runAdapter(context, "flint", taskId, { task, packet: stripStateMetadata(packet), worktree_path: worktree.absolute_path, base_sha: packet.base_sha });
  ...
  const puckResult = runAdapter(context, "puck", taskId, { task, packet: stripStateMetadata(packet), candidate, snapshot: replayRunState(context.repository, context.runId).entities.snapshot[candidate.snapshot_id], worktree_path: worktree.absolute_path });
  ...
  if (packet.vera_required) {
    const veraResult = runAdapter(context, "vera", taskId, { task, packet: stripStateMetadata(packet), candidate, snapshot: replayRunState(context.repository, context.runId).entities.snapshot[candidate.snapshot_id], worktree_path: worktree.absolute_path });
...
function runAdapter(context, role, taskId, payload) {
  const state = replayRunState(context.repository, context.runId);
  const entityId = `ADAPTER-${role.toUpperCase()}-${taskId ?? "RUN"}`;
  ...
  const result = spawnSync(configured.command[0], configured.command.slice(1), { cwd: context.repository, input: JSON.stringify({ protocol_version: RUNNER_VERSION, role, task_id: taskId, run_id: context.runId, ...payload }), encoding: "utf8", timeout: configured.timeout_ms ?? 120_000, maxBuffer: 8 * 1024 * 1024, env: { PATH: process.env.PATH ?? "/usr/bin:/bin", HUSH_RUN_ID: context.runId, HUSH_ROLE: role } });
  ...
  const artifactPath = join(context.repository, ".hush", "runs", context.runId, "artifacts", `${role}-${taskId ?? "run"}.json`);
