...
import { bindGateReport, validateAcceptance, validateCheckCoverage, validateGateReport } from "./evidence.mjs";
...
async function runRook(context, requirements) {
  ...
  for (const task of result.tasks) {
    ...
    const packet = sealPacket(task.packet);
...
async function processTask(context, taskId) {
  ...
  try {
    ...
    if (reuse && fastForward(worktree.absolute_path, reuse.endSha)) {
      ...
      flint = await runAdapter(context, "flint", taskId, { task, packet: stripStateMetadata(packet), worktree_path: worktree.absolute_path, base_sha: packet.base_sha });
    }
...
function validateAndRecordGate(context, gate, taskId, packet, candidate, gateReport) {
  const snapshot = replayRunState(context.repository, context.runId).entities.snapshot[candidate.snapshot_id];
  const report = bindGateReport(gateReport, { gate, packet: stripStateMetadata(packet), snapshot, candidate, runId: context.runId });
