...
import { enqueueCandidate, processNextMerge, recoverIntegrating } from "./merge-queue.mjs";
import { HARNESS_USAGE_KEY } from "./harness-adapter.mjs";
import { sealPacket, validatePacket } from "./packet.mjs";
import { completeRequirementSource, hasBlockingUnknowns, validateRequirementRecord } from "./requirements.mjs";
...
import { mutationPolicyForRequirements, runStrykerPolicyAsync, validateMutationPolicy, validateMutationReport } from "./verification.mjs";
import { allocateWorktree, cleanupWorktree, recoverInterruptedWorktreeLeases, releaseWorktree, warmWorktreeAsync } from "./worktree.mjs";
import { GhAxiAdapter, deliverPullRequest, renderPrPayload } from "./delivery.mjs";
...
async function runWorkflowInternal(options, captureContext) {
  ...
  if (input.resume) {
    ...
    recoverIntegrating({ root: repository, repo: repository, target: context.target, now: context.now });
  }
  ...
  for (const candidate of acceptedCandidates) {
    ...
    const payload = renderPrPayload({ run_id: runId, candidate: acceptedCandidate, requirements: requirements.map((item) => item.requirement_id), checks: integration.integration_record?.checks ?? [], evidence: evidenceIds(accepted, acceptedCandidate), target: context.target, head: context.delivery?.head ?? acceptedCandidate.branch });
    ...
    const deliveryEvent = { event_id: `DELIVERY-${runId}-${candidate.id}`, entity_type: "delivery", entity_id: `PR-${runId}-${candidate.id}`, action: "pr-payload-rendered", actor: "hush", cause: "runner-local-integration", timestamp: context.now, data: { status: "READY_FOR_PR", event_type: "READY_FOR_PR", repository, target: context.target, auto_merge_policy: context.delivery?.auto_merge_policy ?? null, ...delivery } };
    const remoteDelivery = context.delivery?.enabled
      ? deliverPullRequest(repository, runId, deliveryEvent, { adapter: new GhAxiAdapter({ repository, repositorySlug: context.delivery.repository_slug, timeoutMs: context.delivery.timeout_ms }) })
      : (appendStateEvent(repository, runId, deliveryEvent), null);
    if (remoteDelivery) delivery.remote = { status: remoteDelivery.status, pr_id: remoteDelivery.pr_id, response_digest: remoteDelivery.response_digest };
    deliveries.push(delivery);
  }
  const finalStatus = context.delivery?.enabled ? "DELIVERY_PENDING" : "ACCEPTED";
  appendRunStatus(context, finalStatus, "runner-acceptance", { acceptance_record_ids: deliveries.map((item) => `ACC-${item.queue_item_id}`), candidate_ids: deliveries.map((item) => item.candidate_id), integration_ids: deliveries.map((item) => item.integration_id), pr_payload_digests: deliveries.map((item) => item.payload_digest), delivery_status: context.delivery?.enabled ? "PR_OPEN" : "LOCAL_ONLY" });
  return finalize(context, replayRunState(repository, runId), RUN_EXIT_CODES.SUCCESS, finalStatus);
}
...
function loadRunnerConfig(path) {
  ...
  if (config.delivery?.enabled === true) {
    if (typeof config.delivery.repository_slug !== "string" || !/^[^/\s]+\/[^/\s]+$/.test(config.delivery.repository_slug)) throw new RunnerError("config.delivery.repository_slug must be owner/name", RUN_EXIT_CODES.INVALID_INPUT);
    if (typeof config.delivery.head !== "string" || config.delivery.head.length === 0) throw new RunnerError("config.delivery.head is required when delivery is enabled", RUN_EXIT_CODES.INVALID_INPUT);
    if (config.delivery.timeout_ms !== undefined && (!Number.isInteger(config.delivery.timeout_ms) || config.delivery.timeout_ms < 1)) throw new RunnerError("config.delivery.timeout_ms must be positive", RUN_EXIT_CODES.INVALID_INPUT);
  }
...
async function runFable(context) {
  const prdPath = relative(context.repository, context.prdPath);
  const result = await runAdapter(context, "fable", null, { prd_path: prdPath, prd_digest: context.prdDigest, prd_text: context.prdText, source_manifest: context.source });
  ...
  const requirements = [];
  for (const drafted of result.requirements) {
    // Hush owns the PRD bytes, so Hush fills the source digest, location, and quote-back. Fable owns the content.
    const { record: requirement, quoteFound } = completeRequirementSource(drafted, { path: prdPath, text: context.prdText, digest: context.prdDigest });
    if (!quoteFound) throw new RunnerError(`Fable requirement ${drafted?.requirement_id ?? "unknown"} is invalid: quote is not an exact substring of ${prdPath}`, RUN_EXIT_CODES.BLOCKED);
  ...
  appendRunStatus(context, "FABLE_COMPLETE", "fable-complete", { requirement_ids: requirements.map((item) => item.requirement_id).sort() });
  return requirements;
}
...
async function runRook(context, requirements) {
  ...
  for (const task of result.tasks) {
    if (!task.task_id || !Array.isArray(task.requirements) || task.requirements.length === 0) throw new RunnerError("Rook task must include task_id and requirements", RUN_EXIT_CODES.BLOCKED);
    const unknownRequirement = task.requirements.find((requirementId) => !requirementIds.has(requirementId));
    if (unknownRequirement) throw new RunnerError(`Rook references unknown requirement: ${unknownRequirement}`, RUN_EXIT_CODES.BLOCKED);
    // A custom adapter can still send a full packet. A model sends only the planning fields, and Hush builds the packet.
    const draft = task.packet ?? packetFromTask(context, task, requirements);
    for (const requirementId of task.requirements) {
      const requirement = requirements.find((item) => item.requirement_id === requirementId);
      if (!draft.source_refs?.some((source) => source.manifest_digest === requirement.source.digest && source.location === requirement.source.location)) throw new RunnerError(`Rook packet ${draft.packet_id ?? task.task_id} is not bound to requirement source ${requirementId}`, RUN_EXIT_CODES.BLOCKED);
      covered.add(requirementId);
    }
    // Rook drafts the packet and Hush issues it. Hush seals the digest so that a model never has to compute a canonical SHA-256 by hand.
    ...
    if (packetResult.status !== "VALID" || packet.run_id !== context.runId || packet.task_id !== task.task_id) throw new RunnerError(`Rook packet ${packet.packet_id ?? "unknown"} is invalid: ${packetResult.status}${packetResult.issue ? ` (${packetResult.issue.field} ${packetResult.issue.rule})` : ""}`, RUN_EXIT_CODES.BLOCKED, packetResult);
    ...
    appendStateEvent(context.repository, context.runId, { entity_type: "task", entity_id: task.task_id, action: "planned", actor: "rook", cause: "runner-rook", timestamp: context.now, data: { ...task, packet, id: undefined, status: "READY", packet_id: packet.packet_id } });
    packets.push(packet);
  }
...
/**
 * Build the work packet for a Rook task that has only planning fields. Rook decides scope, commands, and checks. Hush
 * fills the identity, source, environment, and lifecycle fields that it owns, because hush.toml says "Hush issues
 * packets". validatePacket still checks every Rook decision. The packet `dependencies` is [] at issuance: a packet
 * dependency must already be VERIFIED, and the scheduler orders tasks by the task-level `dependencies`. The mutation
 * policy comes from the requirements (mutationPolicyForRequirements), not from Rook.
 */
function packetFromTask(context, task, requirements) {
  const taskRequirements = requirements.filter((item) => task.requirements.includes(item.requirement_id));
  const sourceRefs = [...new Map(taskRequirements.map((item) => [`${item.source.digest}\0${item.source.location}`, { snapshot_id: `SRC-${item.requirement_id}`, manifest_digest: item.source.digest, location: item.source.location }])).values()];
  const writePaths = task.write_paths ?? [];
  const veraRequired = task.vera_required !== false;
  const mutationPolicy = mutationPolicyForRequirements(taskRequirements);
  return {
    contract_version: "hec.v1", run_id: context.runId, plan_id: `PLAN-${context.runId}`, task_id: task.task_id, packet_id: `PKT-${task.task_id}`,
    packet_revision: 1, requirement_map_revision: Math.max(1, ...taskRequirements.map((item) => item.revision ?? 1)), target_agent: "flint", base_sha: context.baseSha,
    source_refs: sourceRefs, requirements: task.requirements,
    allowed_paths: task.allowed_paths ?? [], write_paths: writePaths, write_path_inventory: writePaths, allowed_operations: task.allowed_operations ?? [],
    blocked_paths: task.blocked_paths ?? [], exclusive_hubs: task.exclusive_hubs ?? [], dependencies: [],
    required_commands: task.required_commands ?? [], acceptance_checks: task.acceptance_checks ?? [], expected_evidence: task.expected_evidence ?? [],
    environment: context.environment, vera_required: veraRequired, vera_trigger: veraRequired ? "rook-plan" : null,
    attempt: 1, strike_count: 0, expires_at: null, supersedes: [], packet_state: "ACTIVE",
    ...(mutationPolicy.required ? { mutation_policy: mutationPolicy } : {}),
  };
}

/**
 * Run tasks as a pool. When any task finishes, the scheduler admits the next READY task at once, so a finished slot
 * never waits for the slowest task in a batch. dispatchReadyTasks counts RUNNING tasks against max_workers, so the pool
 * never goes over the worker limit. After the first failure, the pool admits no new tasks. It waits for the running
 * tasks to settle and then throws that failure. This is the same failure behavior as the earlier batch barrier.
 */
async function processTasks(context) {
  ...
  for (let loop = 0; loop < maxLoops; loop += 1) {
    if (!failure) {
      ...
      if (complete && running.size === 0) return;
      ...
      for (const { task_id: taskId } of dispatch.dispatched) {
        running.set(taskId, processTask(context, taskId).then(() => ({ taskId }), (error) => ({ taskId, error })));
      }
      if (running.size === 0) {
    ...
    if (running.size === 0) throw failure;
    const settled = await Promise.race(running.values());
    running.delete(settled.taskId);
    if (settled.error && !failure) failure = settled.error;
  }
...
async function processTask(context, taskId) {
  ...
  // Flint, Puck, and Vera get the requirement text. Vera cannot check alignment against requirement IDs only.
  const taskRequirements = (task.requirements ?? []).map((id) => state.entities.requirement?.[id]).filter(Boolean);
  let worktree = null;
  let allocated = false;
  let expectedHead = packet.base_sha;
  try {
    const retryWorktreeId = Number(task.attempt ?? 1) > 1 ? `WT-${context.runId}-${taskId}-A${task.attempt}` : undefined;
    const warm = await warmWorktreeAsync({ repo: context.repository, baseSha: packet.base_sha, runId: context.runId, taskId, worktreeId: retryWorktreeId, profile: context.setup_profile, packet: stripStateMetadata(packet), root: context.repository, timestamp: context.now });
    worktree = warm.worktree ?? null;
    ...
    worktree = allocateWorktree({ root: context.repository, repo: context.repository, runId: context.runId, taskId, packet: stripStateMetadata(packet), now: context.now, worktreeId: warm.worktree.worktree_id });
    allocated = true;
    const reuse = reusableFlintWork(state, task);
    let flint;
    if (reuse && fastForward(worktree.absolute_path, reuse.endSha)) {
      flint = reuse.output;
      appendStateEvent(context.repository, context.runId, { entity_type: "task", entity_id: taskId, action: "flint-reused", actor: "hush", cause: "runner-resume", timestamp: context.now, data: { reused_candidate_id: reuse.candidateId, reused_end_sha: reuse.endSha, reused_flint_artifact: reuse.artifactPath } });
    } else {
      flint = await runAdapter(context, "flint", taskId, { task, packet: stripStateMetadata(packet), requirements: taskRequirements, worktree_path: worktree.absolute_path, base_sha: packet.base_sha });
    }
    let candidate = freezeCandidate(context, task, packet, worktree.absolute_path, flint.candidate ?? flint);
    expectedHead = candidate.end_sha;
    ...
    if (mutationPolicy.required) {
      ...
      let mutation;
      try {
        mutation = await runStrykerPolicyAsync({ cwd: worktree.absolute_path, command: packet.mutation_policy?.command ?? context.mutation_command, reportPath: packet.mutation_policy?.report_path ?? "reports/mutation.json", packet: stripStateMetadata(packet), candidate, snapshot, changedPaths: candidate.changed_paths, root: context.repository, runId: context.runId, now: context.now });
      } catch (error) {
    ...
    const puckResult = await runAdapter(context, "puck", taskId, { task, packet: stripStateMetadata(packet), requirements: taskRequirements, candidate, snapshot: replayRunState(context.repository, context.runId).entities.snapshot[candidate.snapshot_id], worktree_path: worktree.absolute_path });
    ...
    if (packet.vera_required) {
      const veraResult = await runAdapter(context, "vera", taskId, { task, packet: stripStateMetadata(packet), requirements: taskRequirements, candidate, snapshot: replayRunState(context.repository, context.runId).entities.snapshot[candidate.snapshot_id], worktree_path: worktree.absolute_path });
    ...
    if (worktree) {
      try {
        const cleanup = allocated
          ? releaseWorktree({ root: context.repository, runId: context.runId, worktreeId: worktree.worktree_id, cleanup: true, expectedHead, now: context.now })
          : cleanupWorktree({ root: context.repository, runId: context.runId, worktreeId: worktree.worktree_id, expectedHead: packet.base_sha, now: context.now });
        if (cleanup.status === "BLOCKED") appendStateEvent(context.repository, context.runId, { entity_type: "finding", entity_id: `WORKTREE-CLEANUP-${taskId}`, action: "cleanup-blocked", actor: "hush", cause: cleanup.code ?? "WORKTREE_CLEANUP_BLOCKED", timestamp: context.now, data: { task_id: taskId, worktree_id: worktree.worktree_id, blocking: false, reason: cleanup.code ?? "WORKTREE_CLEANUP_BLOCKED" } });
      } catch (error) {
        appendStateEvent(context.repository, context.runId, { entity_type: "finding", entity_id: `WORKTREE-CLEANUP-${taskId}`, action: "cleanup-error", actor: "hush", cause: "WORKTREE_CLEANUP_ERROR", timestamp: context.now, data: { task_id: taskId, worktree_id: worktree.worktree_id, blocking: false, error: error.message } });
      }
    }
  }
}
...
/**
 * Find Flint work that a resumed task can reuse. This applies only when an interrupt happened after Flint finished and
 * Hush froze the candidate (retry_reason VERIFICATION_INTERRUPTED). In that case the candidate commit and Flint's
 * saved output are still on disk, so a new Flint call would repeat finished work. The function returns null when any
 * part is missing, and the runner then calls Flint as usual. freezeCandidate recomputes every digest from the
 * fast-forwarded worktree, so the gates still bind to the actual tree.
 */
function reusableFlintWork(state, task) {
  if (task.retry_reason !== "VERIFICATION_INTERRUPTED") return null;
  const candidate = Object.values(state.entities.candidate ?? {}).find((item) => item.task_id === task.id && typeof item.end_sha === "string");
  const artifact = Object.values(state.entities.evidence ?? {}).filter((item) => item.role === "flint" && item.task_id === task.id && item.status === "COMPLETED" && item.artifact_path && existsSync(item.artifact_path)).at(-1);
  if (!candidate || !artifact) return null;
  return { candidateId: candidate.candidate_id, endSha: candidate.end_sha, artifactPath: artifact.artifact_path, output: JSON.parse(readFileSync(artifact.artifact_path, "utf8")) };
}

/** Move a fresh task worktree to an earlier candidate commit. This works only as a fast-forward from the packet base. */
function fastForward(worktreePath, endSha) {
  return spawnSync("git", ["merge", "--ff-only", "--quiet", endSha], { cwd: worktreePath, encoding: "utf8" }).status === 0;
}
...
async function runAdapter(context, role, taskId, payload) {
  ...
  // The harness-adapter bridge prints its errors as JSON on stdout. Fall back to stdout so the real cause is not lost.
  if (result.error || result.status !== 0) throw new RunnerError(`${role} adapter failed: ${result.error?.message || result.stderr?.trim() || result.stdout?.trim().slice(0, 2000) || `exit ${result.status}`}`, RUN_EXIT_CODES.FAILED, { role, task_id: taskId });
  let output;
  ...
  let usage = null;
  if (output && typeof output === "object" && !Array.isArray(output) && Object.hasOwn(output, HARNESS_USAGE_KEY)) {
    usage = usageRecord(output[HARNESS_USAGE_KEY]);
    output = { ...output };
    delete output[HARNESS_USAGE_KEY];
  }
...
const USAGE_TOKEN_FIELDS = ["input_tokens", "cache_read_tokens", "cache_write_tokens", "output_tokens"];

/** Keep only the known numeric usage fields. The value comes from adapter stdout, so this function does not trust its shape. */
function usageRecord(value) {
  if (!value || typeof value !== "object") return null;
  const record = Object.fromEntries(USAGE_TOKEN_FIELDS.map((field) => [field, Number.isFinite(Number(value[field])) ? Math.max(0, Number(value[field])) : 0]));
  record.cost_usd = value.cost_usd === null || value.cost_usd === undefined || !Number.isFinite(Number(value.cost_usd)) ? null : Number(value.cost_usd);
  return record;
}

/**
 * Add up adapter calls, wall time, and harness token usage for each role and for the whole run.
 * `calls_with_usage` shows how many calls reported usage. Custom adapters and older bridges report none.
 */
function usageSummary(state) {
  const empty = () => ({ calls: 0, calls_with_usage: 0, duration_ms: 0, ...Object.fromEntries(USAGE_TOKEN_FIELDS.map((field) => [field, 0])), cost_usd: null });
  const byRole = {};
  const total = empty();
  for (const record of Object.values(state.entities.evidence ?? {})) {
    if (record.status !== "COMPLETED" || !record.role) continue;
    for (const row of [(byRole[record.role] ??= empty()), total]) {
      row.calls += 1;
      row.duration_ms += Number(record.duration_ms) || 0;
      if (!record.usage) continue;
      row.calls_with_usage += 1;
      for (const field of USAGE_TOKEN_FIELDS) row[field] += Number(record.usage[field]) || 0;
      if (record.usage.cost_usd !== null && record.usage.cost_usd !== undefined) row.cost_usd = (row.cost_usd ?? 0) + Number(record.usage.cost_usd);
    }
  }
  return { by_role: byRole, total };
}
