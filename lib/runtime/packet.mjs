...
export function validatePacket(packet, options = {}) {
  ...
  const environment = validateEnvironment(packet.environment, { ...(options.environment ?? {}), root: options.root, repository: options.repository, now: options.now, requireFreshInventory: packet.environment?.hazard_inventory_required === true });
...
export function validatePacketShape(packet) {
  ...
  if (packet.write_path_inventory !== undefined && (!Array.isArray(packet.write_path_inventory) || new Set(packet.write_path_inventory).size !== packet.write_path_inventory.length || packet.write_path_inventory.some((path) => !packet.write_paths.includes(path) || !packet.allowed_paths.includes(path)) || packet.write_path_inventory.length !== packet.write_paths.length)) return { field: "write_path_inventory", rule: "must exactly enumerate write_paths" };
  ...
  if (packet.mutation_policy !== undefined && (!isPlainObject(packet.mutation_policy) || typeof packet.mutation_policy.required !== "boolean")) return { field: "mutation_policy", rule: "must declare required boolean" };
  return null;
}
