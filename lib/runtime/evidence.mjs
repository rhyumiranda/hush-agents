...
/** Fields Hush fills in a gate report. The model does not copy them or compute them. */
export const HUSH_BOUND_GATE_FIELDS = Object.freeze(["report_id", "gate", "phase", "packet_id", "source_manifest_digest", "candidate_id", "candidate_patch_digest", "candidate_tree_digest", "candidate_diff_digest", "snapshot_id", "gate_run_id", "created_by", "report_digest"]);

/**
 * Bind a gate report to the packet, snapshot, and candidate that Hush sent to the gate.
 * - Hush fills each missing binding field from its own records. A field that is absent, null, or "" counts as missing.
 * - Hush keeps each binding field that the gate supplied. A wrong value still fails validateGateReport.
 * - Hush always recomputes report_digest. The report comes directly from the gate process that Hush started,
 *   so the digest seals the record that Hush stores. validateAcceptance checks that seal again later.
 * Before this function existed, a model had to compute a canonical SHA-256 by hand. The live run needed 7 gate calls
 * where 2 were enough (docs/live-agent-shipping-runtime-report.md).
 */
export function bindGateReport(report, { gate, packet, snapshot, candidate, runId }) {
  if (!report || typeof report !== "object" || Array.isArray(report)) return report;
  const GATE = String(gate).toUpperCase();
  const bound = {
    report_id: `${GATE}-${candidate?.candidate_id}`,
    gate: GATE,
    phase: "PRE_INTEGRATION",
    packet_id: packet?.packet_id,
    source_manifest_digest: packet?.source_refs?.[0]?.manifest_digest,
    candidate_id: candidate?.candidate_id,
    candidate_patch_digest: candidate?.patch_digest,
    candidate_tree_digest: snapshot?.tree_digest,
    candidate_diff_digest: snapshot?.diff_digest,
    snapshot_id: snapshot?.snapshot_id,
    gate_run_id: `${runId}-${GATE}-${candidate?.candidate_id}`,
    created_by: String(gate).toLowerCase(),
    ...Object.fromEntries(Object.entries(report).filter(([key, value]) => !(HUSH_BOUND_GATE_FIELDS.includes(key) && (value === null || value === undefined || value === "")))),
  };
  delete bound.report_digest;
  return { ...bound, report_digest: computeReportDigest(bound) };
}
