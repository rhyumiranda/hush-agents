...
export function validateRequirementRecord(record, options = {}) {
  ...
  if (options.sourceBytes !== undefined) {
    const verified = validateSourceBytes({ sourceBytes: options.sourceBytes, quote: record.quote, location: record.source.location, digest: record.source.digest, quoteBytes: record.quote_bytes });
    if (!verified.valid) return issue("quote", verified.reason, { actualDigest: verified.actualDigest });
  } else if (options.sourceText !== undefined) {
...
/**
 * Fill the source bookkeeping of a Fable requirement from the PRD that Hush read itself: `revision` (1 when it is
 * missing), `source`, `source_discovery`, and `quote_back`. A model cannot compute the SHA-256 of the PRD, and the
 * validator checks these fields against the PRD bytes in any case. The location is the PRD line where the quote starts.
 * Returns `{ record, quoteFound }`. When the quote is not an exact substring of the PRD, the function returns the
 * record unchanged and `quoteFound: false`, so the caller can report the real problem.
 */
export function completeRequirementSource(record, { path, text, digest }) {
  if (!record || typeof record !== "object" || Array.isArray(record)) return { record, quoteFound: false };
  const offset = typeof record.quote === "string" && record.quote.length > 0 ? text.indexOf(record.quote) : -1;
  if (offset < 0) return { record, quoteFound: false };
  const location = `${path}:${text.slice(0, offset).split("\n").length}`;
  const source = { path, location, digest };
  return {
    record: {
      ...record,
      revision: Number.isInteger(record.revision) && record.revision > 0 ? record.revision : 1,
      source,
      source_discovery: { method: "hush-prd-read", ...source, status: "VERIFIED" },
      quote_back: { verified: true, quote: record.quote, location, digest },
    },
    quoteFound: true,
  };
}
export function validateSourceQuote({ sourceText, quote, location, digest }) {
  return validateSourceBytes({ sourceBytes: Buffer.from(sourceText, "utf8"), quote, location, digest });
}

export function validateSourceBytes({ sourceBytes, quote, quoteBytes, location, digest }) {
  const bytes = Buffer.isBuffer(sourceBytes) ? sourceBytes : Buffer.from(sourceBytes);
  const actualDigest = `sha256:${createHash("sha256").update(bytes).digest("hex")}`;
  if (actualDigest !== digest) return { valid: false, reason: "source byte digest mismatch", actualDigest };
  if (!location || typeof location !== "string") return { valid: false, reason: "source location is required" };
  const exactQuote = quoteBytes === undefined ? Buffer.from(String(quote ?? ""), "utf8") : Buffer.from(quoteBytes, "base64");
  if (exactQuote.length === 0 || !bytes.includes(exactQuote)) return { valid: false, reason: "quote is not an exact source byte substring" };
...
export function discoverRequirementSource(record, { root = ".", sourceText } = {}) {
  ...
  try {
    ...
    return validateSourceBytes({ sourceBytes: bytes, quote: record.quote, quoteBytes: record.quote_bytes, location: record.source.location, digest: record.source.digest });
  } catch (error) {
